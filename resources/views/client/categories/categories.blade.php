...
<div class="category-top">
    @if (isset($category))
       <a href="{{route('client_index')}}">{{__('Home')}} </a>> {{ $category->name }}
    @else
...
<section class="category-sub">
    @foreach ($latestSubCategories as $subCategory)
        @php
            ...
            $firstProduct = $subCategory->products->last();
            if ($firstProduct && $firstProduct->image) {
                $imagePaths = explode(',', $firstProduct->image);
                $latestImagePath = end($imagePaths);
            }
        @endphp
        <a href="{{ route('client.subcategories', ['id' => $subCategory->id]) }}" class="category-img" style="background-image: url('{{ asset("storage/images/admin/product/" . $latestImagePath) }}')">
            <div class="category-sub-name">{{ $subCategory->name }}</div>
        </a>
    @endforeach
</section>

...
@section('content')
<legend>{{ __('Custom Orders') }}</legend>
<div class="mt-2">
...
<section class="products">
    <div class="products-topbar">
        ...
        <form class="products-search" method="GET" action="{{ route('search.orders') }}">
            @csrf
            <input type="search" name="custom-search" placeholder="Search for Custom Orders..." value="{{ request('custom-search') }}">
            ...
            <tbody>
                @foreach ($orders as $order)
                    <tr>
                        ...
                        <td>
                            <button class="action btn btn-primary" onclick="location.href='{{ route('show.custom.order', $order->id) }}'">
                                <i class="fas fa-eye"></i>
                            </button>
                            {!! Form::open([
                                'method' => 'DELETE',
                                'route' => ['delete.custom.order', $order->id],
